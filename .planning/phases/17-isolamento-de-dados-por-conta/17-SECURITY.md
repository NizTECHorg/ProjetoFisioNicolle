...
| React Query no mesmo separador | Logout e troca de uid limpam o cache. A chave da consulta inclui o uid, então a conta seguinte não lê a entrada da anterior | Quadro, pacientes, sessões e prazos |
...
| T-17-05 | Elevation of privilege | Filtro só na UI | mitigate | Cache limpo no logout e na troca de usuário. Chaves clínicas terminam no uid. O cliente não filtra dono; a parede é o RLS | closed |
...
| T-17-05 | `src/providers/AuthProvider.tsx`, `src/hooks/useAccountScope.ts`, `src/hooks/useClinic.ts`, `src/hooks/usePatients.ts` | `signOut` chama `queryClient.clear()` antes do sign-out. Mudança de `userId` depois da sessão carregada também limpa o cache. `useBoard` usa `['board', userId]`; a lista de pacientes usa `['patients', 'list', userId]`. Consultas clínicas ficam `enabled` só com uid. `listBoard`, `listDueCards` e `listPatients` não filtram `owner_id`. `staleTime` de 60s permanece e não cruza contas, porque a chave da conta B não lê a entrada da conta A. | No código. |
...
| 2026-09-23 | 8 | 8 | 0 | gsd-security-auditor |
...
T-17-05 permanecia aberta: os selects não filtram dono no cliente, como o plano pediu, mas o cache do React Query mostrava quadro, pacientes e prazos da conta anterior sem consultar o Postgres.

### Security Audit 2026-09-23 (reexecução)
...
T-17-05 fechada. `queryClient.clear()` roda no logout e quando o uid muda. As chaves clínicas incluem o uid. ASVS nível 1. `block_on: high`.
